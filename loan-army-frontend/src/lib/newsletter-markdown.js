...
export function convertNewsletterToMarkdown(newsletter, options = {}) {
    ...
    // Header
    const title = content.title || newsletter.title || 'Loan Watch'
    lines.push(`# ${title}`)
    lines.push('')
    ...
    for (const section of sections) {
        ...
        for (const item of items) {
            ...
            if (loanTeam) {
                lines.push(`*On loan at ${loanTeam}*`)
            }
            lines.push('')
    ...
    lines.push('---')
    lines.push('')
    lines.push('*Generated by [The Academy Watch](https://goonloan.com) — Weekly loan watch newsletters for football fans*')
...
export function convertNewsletterToCompactMarkdown(newsletter) {
    ...
    lines.push('---')
    lines.push('*via [The Academy Watch](https://goonloan.com)*')
