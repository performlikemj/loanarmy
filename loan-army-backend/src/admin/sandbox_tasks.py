...
from src.models.league import League, LoanFlag, LoanedPlayer, SupplementalLoan, Team, Player
from src.utils.brave_loans import BraveLoanCollection, collect_loans_from_brave
...
from unicodedata import normalize as _ud_normalize, combining as _ud_combining
...
def run_task(task_id: str, payload: MutableMapping[str, Any], context: SandboxContext) -> Dict[str, Any]:
    ...
    if not isinstance(result, SandboxTaskResult):
        raise TaskExecutionError(
            f"Task '{task_id}' returned unsupported response type: {type(result)!r}"
        )
...
def _task_update_sofascore_id(
    ...
    if normalized_sofa:
        existing = (
            ...
            .filter(Player.sofascore_id == normalized_sofa, Player.player_id != player_id)
            .first()
        )
        if existing:
...
def _diff_loan_rows(
    *,
    ...
    logger.info(
        ...
        getattr(team, 'name', None), season_year, len(rows or []), apply_changes, use_openai, data_source,
    )
    if not rows:
        return [], 0

    query = context.db_session.query(LoanedPlayer).filter(
        LoanedPlayer.window_key.like(f"{season_year}-%")
    )
    if not run_all_teams and team is not None:
        query = query.filter(LoanedPlayer.primary_team_id == team.id)
        ...
        if (use_openai or apply_changes) and not present:
            ...
            if apply_changes and classification.get('valid') and team is not None:
                ...
                if not existing_sup:
                    ...
                    try:
                        lp_match = (
                            context.db_session.query(LoanedPlayer)
                            .filter(LoanedPlayer.primary_team_id == team.id)
                            .filter(func.lower(LoanedPlayer.player_name) == resolved_player.lower())
...
def _task_check_missing_loanees(
    ...
    query = context.db_session.query(LoanFlag)
    if not include_resolved:
        query = query.filter(LoanFlag.status != "resolved")

    flags: List[LoanFlag] = query.order_by(LoanFlag.created_at.desc()).all()

    missing: List[Dict[str, Any]] = []
    for flag in flags:
        exists = (
            context.db_session.query(LoanedPlayer)
            .filter(LoanedPlayer.player_id == flag.player_api_id)
            .first()
        )
        if exists:
            continue
    ...
    summary = (
        f"Potential missing loanees: {len(missing)}"
        if missing
        else "No missing loanees detected"
    )
...
def _task_compare_player_stats(
    ...
    loan = (
        context.db_session.query(LoanedPlayer)
        .filter(LoanedPlayer.player_id == player_id_int)
        .first()
    )
    if not loan:
    ...
    for entry in statistics:
        ...
        appearances = (
            games_block.get("appearances")
            if "appearances" in games_block
            else games_block.get("appearences")
        )
...
def _task_fetch_player_profile(
    ...
    if player_id_param:
        ...
        summary = (
            f"Player {player_id_int}: profile for {player_name}" if player_name else f"Player {player_id_int}: profile not found"
        )
        return SandboxTaskResult(
            status="ok",
            summary=summary,
    ...
    summary = (
        f"Found {len(results)} matches for '{search_query}'"
        if results
        else f"No matches for '{search_query}'"
    )
    return SandboxTaskResult(
        status="ok",
        summary=summary,
...
def _task_bulk_resolve_supplemental_api_ids(
    ...
    for r in rows:
        ...
        # Strategy 1: unique match in LoanedPlayer (exact OR normalized OR last-name+team)
        lp_ids: list[int] = []
        lp_ids_norm: list[int] = []
        lp_ids_last_team: list[int] = []
        try:
            lp_rows_full: list[LoanedPlayer] = (
                context.db_session.query(LoanedPlayer)
                .filter(LoanedPlayer.primary_team_id == team.id)
                .all()
            )
...
def _task_wiki_loan_diff(
    ...
    if use_api_roster and not run_all_teams:
        if not context.api_client:
            ...
            logger.info('[wiki-loans] fetching API roster team_id=%s season=%s', getattr(team, 'team_id', None), season_year)
            try:
            ...
            logger.info('[wiki-loans] mapped roster players=%s for team=%s season=%s', api_roster_count, team.name, season_year)
            if player_payloads:
                ...
                logger.info('[wiki-loans] wiki rows added from roster scan=%s total=%s', len(wiki_rows) - before_rows, len(wiki_rows))
...
def _task_brave_loan_diff(
    ...
    collection_lookup: dict[str, BraveLoanCollection] = {}
    ...
    for team_obj in teams_to_process:
        ...
        try:
            collection = collect_loans_from_brave(
                team_name=team_name,
                season_year=season_year,
    ...
    search_results_by_team = {
        team_name: collection_lookup[team_name].results for team_name in collection_lookup
    }
    ...
    return SandboxTaskResult(
        ...
        payload={
            ...
            'search_results': search_results_by_team if run_all_teams else next(iter(collection_lookup.values())).results if collection_lookup else [],
        },
        meta={
...
def _task_scan_duplicate_loans(
    ...
    session = context.db_session
    query = session.query(LoanedPlayer)
    if team:
        query = query.filter(LoanedPlayer.primary_team_id == team.id)
    if active_only:
        query = query.filter(LoanedPlayer.is_active.is_(True))
    ...
    if season_year is not None:
        ...
        query = query.filter(LoanedPlayer.window_key.like(f"{season_slug}%"))

    rows = query.order_by(
        LoanedPlayer.primary_team_id.asc(),
        LoanedPlayer.player_id.asc(),
        LoanedPlayer.updated_at.desc(),
    ).all()

    grouped: dict[tuple[int, int], list[LoanedPlayer]] = defaultdict(list)
    for row in rows:
        grouped[(row.primary_team_id, row.player_id)].append(row)

    def _sort_key(item: LoanedPlayer):
    ...
    summary = (
        f"Found {len(duplicates)} player(s) with duplicate loan rows"
        if duplicates
        else 'No duplicate loans detected'
    )
